# -*- coding: utf-8 -*-
"""Generate Chinese Word analysis report for GFP Protein Design project."""
...
info.add_run("训练数据文件数：10 | 综合数据表：7\n").font.size = Pt(10)
...
doc.add_paragraph(
    ...
    "旨在为蛋白质设计项目提供全面的数据支撑和设计指导。"
    "数据涵盖了GFP的序列、荧光亮度、光学性质、热稳定性以及突变效应等多个维度。"
)
...
add_table(doc, ["数据集", "数据量", "来源", "主要内容"], overview_rows)
doc.add_paragraph()
...
ref_rows = [
    ...
    ["cgreGFP", "226", "Clytia gregaria（水蝨）", "2HPW",
     "较短序列，与avGFP序列同源性较低"],
    ["ppluGFP", "224", "Pontellina plumata（桥足类）", "2G6X",
     "最短的GFP变体之一，序列差异大"],
    ["EGFP", "239", "avGFP工程化(F64L/S65T)", "-",
     "增强型GFP，488nm激发，亮度提升，37°C成熟好"],
]
add_table(doc, ["蛋白质", "长度(aa)", "来源", "推荐PDB", "特点"], ref_rows)
doc.add_paragraph()
...
doc.add_paragraph(
    ...
    f"关键发现：训练数据集中的变体亮度普遍低于野生型avGFP"
    f"（WT亮度={WT_BRIGHT}），"
    f"表明大多数单位点突变对亮度有负面影响。"
    f"这说明GFP的荧光活性对其序列高度优化，"
...
add_table(doc, ["蛋白质变体", "Tm(°C)", "ΔTm(°C)", "关键突变", "方法"], therm_rows)
doc.add_paragraph()
...
doc.add_paragraph(
    "关键发现：\n"
    "• avGFP野生型的Tm约为78°C，具有较高的内在热稳定性\n"
    "• sfGFP通过多个折叠增强突变(S30R/Y39N/F64L/S65T/F99S/M153T/V163A)将Tm提高至~83°C，"
    "同时显著提高了折叠效率和表达水平\n"
    "• S65T突变在EGFP和sfGFP中均出现，该突变不仅优化光谱性质，"
    "还改善了37°C下的折叠效率\n"
    "• F64L和S65T是EGFP的核心突变：增强37°C折叠、红移激发峰至488nm\n"
    "• V163A和M153T增强了37°C下的折叠效率，是\"cycle 3 GFP\"的核心突变\n\n"
    "设计启示：\n"
...
key_muts = [
    ["S30R", "β-折叠S1", "折叠增强",
     "形成新的盐桥网络(E32-R30-E17-R122-E115)，折叠速率+3.5倍，耐变性能力增强"],
    ["Y39N", "β-折叠", "表达优化",
     "密码子优化，折叠动力学改善"],
    ["F46L", "β-折叠", "折叠增强",
     "Venus/R10-3关键突变，改善成熟效率"],
    ["F64L", "生色团附近", "折叠+37°C成熟",
     "EGFP核心突变，增强37°C折叠"],
    ["S65T/G/A", "生色团核心", "光谱优化",
    ...
    ["F99S", "表面环", "折叠增强",
     "cycle 3 GFP/alphaGFP核心突变，改善37°C折叠"],
    ...
    ["T203Y/I/H/W/F", "生色团环境", "光谱核心",
     "Y=YFP变体(YFP/Citrinine/Venus); I=Sapphire(大Stokes位移); H/W/F=红移"],
    ["A206K", "二聚化界面", "单体化",
     "消除二聚化，产生单体荧光蛋白(mGFP)"],
    ["E222G", "生色团环境", "光谱微调",
     "抑制399nm激发峰，保留475nm峰"],
...
add_table(doc, ["位点", "区域", "重要性", "已知有益突变", "避免突变", "说明"], high_sites)
doc.add_paragraph()
...
milestones = [
    ["1994-1996", "WT GFP表征",
     "avGFP序列、晶体结构和基本光谱性质被阐明。"
     "发现S65T突变将激发峰红移至488nm",
     "Tsien, Prasher, Remington等"],
    ["1996-1998", "颜色扩展",
     "Y66H→BFP(蓝色); Y66W→CFP(青色); T203Y→YFP(黄色)。"
     "建立了GFP颜色调色板",
     "Tsien实验室 (Roger Tsien获2008诺贝尔化学奖)"],
    ["1998-2000", "折叠优化 (cycle 3 GFP/EGFP)",
     "F99S/M153T/V163A→cycle 3 GFP; F64L/S65T→EGFP。改善37°C折叠",
     "Crameri, Cormack, Tsien等"],
    ["2001-2005", "单体化和Venus/Cerulean",
     "A206K单体化; Venus(YFP优化)和Cerulean(CFP优化)。"
     "量子产率提升2.5倍",
     "Miyawaki, Piston, Campbell等"],
    ["2006", "sfGFP (superfolder GFP)",
     "S30R/Y39N/F64L/S65T/F99S/M153T/V163A组合→超折叠GFP。"
     "耐变性、高表达",
     "Pedelacq, Waldo等"],
...
add_table(doc, ["年代", "里程碑", "核心贡献", "主要研究者"], milestones)
doc.add_paragraph()
...
add_table(doc, ["参数", "优先级", "关键位点", "优化策略"], priority_rows)
doc.add_paragraph()

doc.add_heading("9.3 设计规避清单", level=2)
doc.add_paragraph(
    "以下是在蛋白质设计中应避免的操作：\n\n"
    "1. 避免在β-折叠核心区域引入Pro或Gly（破坏二级结构）\n"
...
add_table(doc, ["设计目标", "起始骨架", "核心突变组合", "预期效果"], templates_rows)
doc.add_page_break()
...
add_table(doc, ["序号", "源文件", "类型", "记录数", "关键内容"], appendix_rows)
